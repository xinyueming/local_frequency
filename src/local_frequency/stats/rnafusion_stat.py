...
def read_rnafusion_files(sour_path: str) -> list[str]:
    """递归查找 RNA fusion xls 文件（*.tsv.redup.xls）"""
    files = []
    regex = re.compile(r".*redup\.xls$")
    ...
    logger.info("找到 %d 个 RNA fusion 文件", len(files))
    return files
...
def stat_rnafusion(data_dir: str, output_dir: str, sample_prefix: str) -> dict:
    ...
    """
    files = read_rnafusion_files(data_dir)
    if not files:
        logger.warning("未找到 RNA fusion 文件: %s", data_dir)
    ...
    for fi in files:
        ...
        for line in content:
            ...
            exon_parts = str(line.get("exon", "")).split("-")
            ls1 = [
    ...
    logger.info("RNA Fusion 统计完成: %d 个样本 → %s", total, out_file)
